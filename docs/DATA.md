...
Every record also carries a `name_kn_verified` boolean: `true` when the Kannada spelling came
from an actual Kannada-language source (currently just the 30-name 2020 Kempegowda batch, whose
Kannada spellings were read directly off the Kannada Oneindia article rather than generated),
`false` everywhere else. The site does not use this flag to decide whether to *show* Kannada —
`displayName()` in `js/app.js` shows `name_kn` regardless of verification status, since a
best-effort rendering beats falling back to English. The flag exists to drive the review
workflow: `kannada_names_review.xlsx` (generated one-off, not checked into the repo) lists every
recipient with their current `name_kn` and an empty column for hand corrections, sorted
unverified-first, for manual review and re-import.

A caution from experience: for the Kempegowda 2020 batch, an earlier full-dataset
transliteration pass **overwrote** the already-correct, source-read Kannada spellings with the
algorithmic engine's guess, because it ran indiscriminately over every record's English `name`
field without checking whether a verified Kannada spelling already existed. Fixed by restoring
the original spellings and marking them verified. Any future bulk transliteration pass must
skip records where `name_kn_verified` is already `true`.

For the large majority of recipients (no Wikipedia article, or no Kannada one), `name_kn` is
**machine-transliterated**, not a hand-verified spelling. It was generated by a small rule-based
